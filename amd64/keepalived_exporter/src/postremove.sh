...
case "$1" in
    purge)
        if [ -L /etc/systemd/system/multi-user.target.wants/keepalived_exporter.service ]; then
            rm -f /etc/systemd/system/multi-user.target.wants/keepalived_exporter.service || exit 1
        fi
