...
case "$1" in
    purge)
        if [ -L /etc/systemd/system/multi-user.target.wants/pgbackrest_exporter.service ]; then
            rm -f /etc/systemd/system/multi-user.target.wants/pgbackrest_exporter.service || exit 1
        fi
