...
This recipe builds the exact `cargo-pgrx` 0.19.2 Cargo plugin slot from the
published `.crate`, its `Cargo.lock`, and a checksum-pinned vendor archive.

```text
/usr/lib/pgsty/cargo-pgrx/0.19.2/bin/cargo-pgrx
/usr/bin/cargo-pgrx-0192
```

The wrapper uses the single current `rust-toolchain` package at
`/usr/lib/pgsty/rust` and defaults `PGRX_HOME` to `$HOME/.pgrx/0.19.2`.
